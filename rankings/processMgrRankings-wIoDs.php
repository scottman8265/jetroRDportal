<?php
/**
 * make sure correct regions are set for sheet creations on lines 345, 346 & 347
 * make sure branch SQL Query is set to correct location (i.e. "WC", "EC", "MW") in getBranchInfo function
 * make sure queries are correct for 3 sql variables in getAuditScores function
...
function getBranchInfo($lnk) {
	$branchQry = $lnk->query("SELECT branchName, regional, location, branchNum FROM branchInfo.branches WHERE location = 'WC' ORDER BY branchNum ASC");
	$arr       = [];
...
function getAuditScores($lnk) {
	#getting audit scores (make sure audit id limit is correct)
	$sql1 = "SELECT * FROM auditAnalysis.auditscores WHERE auditID > 2509 && rep = 1";
	$qry1 = $lnk->query($sql1);

#getting branches (Make sure location is correct)
	$sql2 = "SELECT branchNum FROM branchInfo.branches WHERE location = 'WC' ORDER BY branchNum ASC";
	$qry2 = $lnk->query($sql2);

#getting audit ids (make sure year is correct)
...
try {
	$rankingQry = $lnk->query("SELECT * FROM staffing.mrRankings WHERE mrID > 8527");
} catch (\Exception $e) {
	...
	foreach ($regionals as $regional => $branches) {
		...
		foreach ($branches as $branch => $mrPos) {
			...
			foreach ($mrPos as $mrPosition => $info) {
				...
				if ($info['name'] !== 'N/A' && $info['tenureObj'] !== 'N/A') {
...
#writes individual department tabs
...
$writer->save("../io/output/WC 2021 Manager Rankings.xlsx");

echo "</br></br>done with file";
